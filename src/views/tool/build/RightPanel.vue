...
const formConf = toRefs(props.formConf);
const showField = toRefs(props.showField);
...
function onDefaultValueInput(str) {
  if (Array.isArray(props.activeData.defaultValue)) {
    // 数组
    activeData.defaultValue = str
      .split(",")
      .map(val => (isNumberStr(val) ? +val : val));
  } else if (["true", "false"].indexOf(str) > -1) {
    // 布尔
    activeData.defaultValue = JSON.parse(str);
  } else {
    // 字符串和数字
    activeData.defaultValue = isNumberStr(str) ? +str : str;
  }
}
...
function setTimeValue(val, type) {
  ...
  activeData.defaultValue = null;
  activeData["value-format"] = valueFormat;
...
function rangeChange(val) {
  activeData.defaultValue = val
    ? [activeData.min, activeData.max]
    : activeData.min;
}

function rateTextChange(val) {
  if (val) activeData["show-score"] = false;
}

function rateScoreChange(val) {
  if (val) activeData["show-text"] = false;
}

function colorFormatChange(val) {
  activeData.defaultValue = null;
  activeData["show-alpha"] = val.indexOf("a") > -1;
  activeData.renderKey = +new Date(); // 更新renderKey,重新渲染该组件
}
...
function setIcon(val) {
  activeData[currentIconModel.value] = val;
}
